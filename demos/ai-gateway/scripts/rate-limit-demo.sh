...
run_tier() {
  ...
  # `|| true`: xargs exits 123 if any child fails, and curl exits non-zero on
  # timeout (28) or refusal (7). Under `set -euo pipefail` that would abort the
  # script before the summary, the premium run and the temp-file cleanup.
  # Those responses still need counting -- that is what the `other` bucket is for.
  seq 1 "${REQUESTS}" | xargs -P "${CONCURRENCY}" -I{} bash -c 'send "$1" "$2"' _ "${tier}" {} >> "${tmp}" || true
...
echo ""
run_tier free
# Not for the budget: every rule gets its own ledger, so the free run's window
# has no bearing on premium's. This is purely to let the model server settle,
# so the premium run is not measured against a saturated queue.
echo "Waiting 65s so Ollama's per-model queue drains before the premium run..."
sleep 65
run_tier premium
