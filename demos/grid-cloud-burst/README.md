...
Token governance is independent from routing. The shared ledger follows the
principal and logical service across consumer gateways and provider changes. Soft
allocation can report over-allocation while allowing a request to complete; hard
enforcement remains a separate policy choice.
...
The scripts require `ffmpeg`, a Chromium-compatible browser for rendering cards,
and
`OPENAI_API_KEY` in the environment. The key is used only by the local
audio-generation script and is never written to the repository.
...
Generated media is ignored by the repository's media rules. The source recording
must
...
- **AI** — [`nerdalert/ai`](https://github.com/nerdalert/ai/tree/burst-routing-v1),
  branch `burst-routing-v1`: gateway filters, weighted selection, token governance,
  and request-path behavior.
- **Grid** — [`nerdalert/grid`](https://github.com/nerdalert/grid/tree/burst-routing-v1),
  branch `burst-routing-v1`: provider discovery, admission, grouping, placement,
  and
  published routing snapshots.
- **Praxis identity** —
  [`nerdalert/praxis`](https://github.com/nerdalert/praxis/tree/poc/authenticated-principal-metadata),
  branch `poc/authenticated-principal-metadata`: authenticated principal metadata.
- **Praxis authority** —
  [`nerdalert/praxis`](https://github.com/nerdalert/praxis/tree/ai-grid-cluster-authority-override),
  branch `ai-grid-cluster-authority-override`: external-provider routing support.
- **Tracing UI** —
  [`nerdalert/praxis-tracing`](https://github.com/nerdalert/praxis-tracing/tree/burst-routing-v1),
  branch `burst-routing-v1`: tracing UI and narrated-demo capture surface.
...
The assembly script checks the source duration, card dimensions, audio presence,
final
...
- Live narration duration: 121.369 seconds, leaving a controlled cushion before
  the
