...
- Use six parts: title card, reactive-burst card, independent-policy card,
  soft-token-limits card, live recording, and outro card.
...
- Review the recording closely and write narration that follows the visible UI state,
  including local routing, application quotas, queue pressure,
  external-provider burst,
  and recovery.
...
- Explain that quota admission happens before provider routing and that the shared
  ledger remains continuous across gateways and provider changes. The distributed
  quota can span gateways, sites, regions, and Kubernetes
  clusters.
- Say “Kubernetes,” “local inference provider,” and “external
  provider.” Do not name cloud, model, or infrastructure vendors unless the
  recording explicitly requires it.
- Do not invent token-type usage, cost, routing, or provider evidence. Say
  that the value is unavailable when the UI does not expose it.
- Use card-based slides with a clean light theme, dark readable text, green local
  state,
...
- Use Playwright to render and inspect each card. Validate the final video duration,
  audio presence, silent lead-in/tail, and narration alignment against the visible
  scene changes before publishing.
